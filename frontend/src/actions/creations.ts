...
export async function updateCreationName(
  ...
  if (type === "Photo to Video") {
    await db.photoToVideoGeneration.updateMany(updateData);
  } else if (type === "Translate Video") {
    await db.videoTranslationGeneration.updateMany(updateData);
  } else if (type === "Change Video Audio") {
    await db.changeVideoAudioGeneration.updateMany(updateData);
  } else {
